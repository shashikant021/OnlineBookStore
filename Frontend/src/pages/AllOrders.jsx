...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        "http://localhost:1000/api/order/get-all-orders",
        { headers }
      );
  ...
  const submitChanges = async (i) => {
    ...
    const response = await axios.put(
      `http://localhost:1000/api/order/update-status/${id}`,
      Values,
      { headers }
    );
        ...
        <div className={`h-auto p-0 md:p-4 text-zinc-100 ${AllOrders && AllOrders.length <=6 ? 'h-[70vh]' : 'h-auto'}`}>
          ...
          {AllOrders.map((items, i) =>
            items?.book ? (
                ...
                <div className="w-[30%] md:w-[16%]">
                  <h1 className="font-semibold">
                    ...
                    >
                      ...
                      <button
                        className="text-green-500 hover:text-pink-700 mx-2"
                        onClick={() => {
                          setOptions(-1);
                          submitChanges(i);
                        }}
              ...
              <div key={i} className="text-red-500 text-center">
                No Orders Found
              </div>
            )
          )}
